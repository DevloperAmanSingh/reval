import {type Inputs} from '../shared/inputs'
...
When a fix is straightforward (<= ~50 new lines), provide it with an inline suggestion block in the format:
<SUGGEST start="$start" end="$end">
replacement content
</SUGGEST>
Include only the replacement code inside the block (no diff markers).
Explain the issue and why the fix works in plain text before the block.
If the fix is too large or uncertain, do not emit a SUGGEST block—describe the problem instead.
When no suggestion is emitted, you may use \`diff\` code blocks, marking changes with \`+\` or \`-\`.
The line number range for comments (and suggestions) must exactly match the range to replace in the new hunk.
...
`
  ...
  renderSummarizeFileDiff(
    ...
    const template = reviewSimpleChanges
      ? this.summarizeFileDiff
      : `${this.summarizeFileDiff}${this.triageFileDiff}`
    return this.renderTemplate(template, inputs)
  }

  renderSummarizeChangesets(inputs: Inputs): string {
    return this.renderTemplate(this.summarizeChangesets, inputs)
  }

  renderSummarize(inputs: Inputs): string {
    return this.renderTemplate(this.summarizePrefix + this.summarize, inputs)
  }

  renderSummarizeShort(inputs: Inputs): string {
    return this.renderTemplate(
      this.summarizePrefix + this.summarizeShort,
      inputs
    )
  }
  ...
  renderSummarizeReleaseNotes(inputs: Inputs): string {
    return this.renderTemplate(
      this.summarizePrefix + this.summarizeReleaseNotes,
      inputs
    )
  }

  renderComment(inputs: Inputs): string {
    return this.renderTemplate(this.comment, inputs)
  }

  renderReviewFileDiff(inputs: Inputs): string {
    return this.renderTemplate(this.reviewFileDiff, inputs)
  }

  private renderTemplate(template: string, inputs: Inputs): string {
    return inputs.render(template)
  }
}
